...
[O sistema de macros do Dart][spec] é um novo recurso importante da linguagem
***atualmente em desenvolvimento*** que adiciona suporte para
[metaprogramação estática][motivation] à linguagem Dart.

Uma macro Dart é um trecho de código definível pelo usuário que recebe outro código como
parâmetros e opera sobre ele em tempo real para criar, modificar ou adicionar declarações.

Você pode pensar no sistema de macros em duas partes: usar macros e escrever macros.
Esta página aborda cada um (em alto nível, já que ***o recurso ainda está em prévia***)
nas seções a seguir:

- [**A macro `JsonCodable`**](#a-macro-jsoncodable):
...
- [**O recurso de macros em geral**](#o-recurso-de-linguagem-macros):
Por que estamos adicionando macros ao Dart, casos de uso motivadores,
benefícios sobre as soluções de geração de código existentes,
e uma visão geral superficial de como a escrita de macros funcionará no futuro,
uma vez que o recurso esteja completo.
...
Isso está disponível no [canal de desenvolvimento do Dart][channel]
...
classes Dart definidas pelo usuário para mapas JSON do tipo `Map<String, Object?>`.
Ela gera dois membros, um método de serialização `toJson`
e um construtor de desserialização `fromJson`.
...
1. Alterne para o [canal de desenvolvimento do Dart][channel] ou o
   [canal master do Flutter][flutter-channel].

2. Execute `dart --version` e certifique-se de ter a versão `3.5.0-152` do Dart ou posterior.

3. Edite a [restrição do SDK][SDK constraint] em seu pubspec para exigir a versão do Dart: `sdk: ^3.5.0-152`.
...
6. Importe o pacote no arquivo em que você planeja usá-lo:
...
A macro inspeciona a classe `User` e deriva as implementações de
`fromJson` e `toJson` usando os campos da classe `User`.

Assim, sem precisar defini-los você mesmo, `toJson` e `fromJson` agora estão
disponíveis para uso em objetos da classe anotada:

```dart
void main() {
 // Dado um JSON arbitrário:
 var userJson = {
   'age': 5,
   'name': 'Roger',
   'username': 'roger1337'
...
Clique no link "**Go to Augmentation**" (Ir para Aumento) que aparece sob a anotação
em sua IDE (suportado no VSCode)
para ver como a macro gera `toJson` e `fromJson`.

Se você alterar algo na classe anotada, poderá ver o aumento gerado
ajustar em tempo real junto com o código do seu aplicativo:

![Um gif lado a lado do aumento gerado sendo atualizado conforme o código que está aumentando é atualizado](/assets/img/language/macro-augmentation.gif)

### Acionar diagnósticos personalizados

A macro `JsonCodable` possui diagnósticos integrados que são emitidos da mesma forma que
diagnósticos da própria linguagem. Por exemplo, se você tentar declarar manualmente
um método `toJson` onde a macro é aplicada, o analisador emitirá
o erro:
...
class HasToJson {
 void [!toJson!]() {}
 // Erro: Não é possível gerar um método toJson devido a este já existente.
}
```

Você pode pesquisar "`DiagnosticMessage`" na [definição de `JsonCodable`][json]
para outros erros que a macro lançará. Por exemplo, estender uma classe que também não é
serializável, ou se os nomes dos campos não corresponderem exatamente aos nomes das chaves no JSON fornecido.

:::note
Para saber mais sobre como usar a macro `JsonCodable`, como tipos de campo compatíveis,
tratamento de nulos e genéricos e muito mais, confira [o README][the README].
:::

[the definition of `JsonCodable`]: {{site.repo.dart.sdk}}/blob/master/pkg/json/lib/json.dart
[the README]: {{site.pub-pkg}}/json

## O recurso de linguagem de macros
...
- **Sem trabalho duplicado** ou recompilação constante prejudicando o desempenho;
  ...
  automaticamente.
- **Não gravadas no disco**, portanto, sem partes de arquivos ou ponteiros para referências geradas;
...
E também muito mais eficiente e muito menos propenso a erros do que escrever manualmente
soluções para esses tipos de problemas.

{% comment %}
Confira estes exemplos que mostram a mesma serialização JSON
implementada de três maneiras diferentes:

- Usando a [`macro JsonCodable`][`JsonCodable` macro].
- Usando o [`pacote de geração de código json_serializable`][`json_serializable` code gen package].
...
boilerplate e, muitas vezes, a necessidade de iterar sobre os campos de uma classe.
...
- **Serialização JSON.** As ferramentas extras necessárias para serializar JSON,
  como o pacote [json_serializable][json_serializable], não são tão eficientes quanto deveriam.
  A macro `JsonCodable` fornece uma maneira muito mais limpa de
  gerar código de serialização; [experimente hoje](#a-macro-jsoncodable).

- **Classes de dados.** O recurso [mais solicitado][most requested] do Dart são as classes de dados
  que fornecem automaticamente um construtor e implementações de `==`,
  `hashCode` e métodos `copyWith()` para cada campo.
...
- **Padrões Flutter detalhados.** Um exemplo é dividir um `build` complexo
  método em uma agregação de classes de widgets menores. É
  melhor para o desempenho e torna o código mais fácil de manter. Infelizmente,
  escrever todas aquelas classes menores requer toneladas de boilerplate, o que desencoraja
  usuários. As macros poderiam potencialmente fornecer uma solução que itera sobre um
  método `build` complexo para gerar classes de widgets menores,
  melhorando muito a produtividade e a qualidade do código Flutter.
  Você pode conferir uma exploração sobre este tópico nesta
  [proposta][stateful-macro] da equipe Flutter.
...
:::important
O recurso de linguagem de macros não é estável e atualmente está por trás de uma
[flag experimental][experimental flag]. A funcionalidade está altamente sujeita a alterações.
Esta seção permanecerá em alto nível até que seja estável.
:::
...
Embora o recurso ainda esteja em desenvolvimento, você pode encontrar a lista completa de
...
a macro para [gerar](#ver-o-código-gerado), bem como quaisquer
[diagnósticos](#acionar-diagnósticos-personalizados) que você deseja que a macro emita.

Em um nível muito alto, escrever macros essencialmente funciona usando métodos builder
para juntar as *propriedades* de uma declaração com *identificadores* nessas
propriedades. A macro coleta essas informações por meio de profunda [introspecção][introspection] do
programa.

As macros ainda estão em desenvolvimento, então isso é o máximo de detalhes que podemos entrar por enquanto.
Se você estiver curioso ou quiser experimentá-lo por trás de uma flag experimental,
a melhor orientação é dar uma olhada na implementação de macros existentes:
...
A data de lançamento estável para macros é desconhecida no momento.
Isso se deve à complexidade de sua implementação.

As macros funcionam por meio de uma profunda [introspecção][introspection] no programa em que
...
Considerando sua aplicação em grandes bases de código, onde várias macros podem
introspecionar e aumentar a base continuamente em diferentes lugares,
o design de [ordenação][ordering] e [fases][phases] de execução é especialmente desafiador
e requer consideração cuidadosa.
