...
ia-translate: true
title: dart:convert
description: Aprenda sobre as principais funcionalidades da biblioteca dart:convert do Dart.
prevpage:
...
possui conversores para JSON e UTF-8, além de suporte para criação de
conversores adicionais. [JSON][] é um formato de texto simples para representar
objetos e coleções estruturados. [UTF-8][] é uma codificação comum de largura variável
que pode representar todos os caracteres do conjunto de caracteres Unicode.
...
chaves string) são diretamente codificáveis em JSON. Objetos List e Map são
...
Para mais exemplos e links para pacotes relacionados a JSON, veja
...
Use `utf8.encode()` para codificar uma string Dart como uma lista de bytes codificados em UTF8:
...
(Latin1). Para mais detalhes, veja a [referência da API para a biblioteca dart:convert.][dart:convert]
