...
Dart possui um rico conjunto de bibliotecas principais que fornecem recursos essenciais para muitas tarefas de programação do dia a dia, como
trabalhar com coleções de objetos (`dart:collection`),
fazer cálculos (`dart:math`),
e codificar/decodificar dados (`dart:convert`).
APIs adicionais estão disponíveis em
[pacotes comumente usados](/resources/useful-packages).

## Tour pelas bibliotecas

Os guias a seguir cobrem como usar as principais funcionalidades das bibliotecas principais do Dart.
Eles fornecem apenas uma visão geral e não são de forma alguma abrangentes.
Sempre que você precisar de mais detalhes sobre uma biblioteca ou seus membros,
consulte a [referência da API Dart.][Dart API]

[dart:core](/libraries/dart-core)
: Tipos embutidos, coleções e outras funcionalidades principais.
...
: I/O para programas que podem usar a Dart VM,
...
Como mencionado, essas páginas são apenas uma visão geral;
elas cobrem apenas algumas bibliotecas dart:*
...
[Bibliotecas da plataforma nativa](#native-platform-libraries) e
[Bibliotecas da plataforma web](#web-platform-libraries) abaixo.

Outros lugares para encontrar informações sobre bibliotecas são o
[site pub.dev]({{site.pub}}) e o
[guia de bibliotecas para desenvolvedores web Dart][webdev libraries].
Você pode encontrar documentação da API para todas as bibliotecas dart:* na
[referência da API Dart][Dart API] ou, se você estiver usando Flutter,
na [referência da API Flutter][api-flutter].
...
| Library                                       | Notes                         |
|-----------------------------------------------|-------------------------------|
| [`dart:core`][dart-core]<br>Tipos embutidos, coleções e outras funcionalidades principais para todo programa Dart. | |
...
| [`dart:collection`][dart-collection], [`package:collection`][package-collection]<br>Classes e utilitários que complementam o suporte a coleções em `dart:core`.<br>`package:collection` fornece implementações de coleções adicionais e funções para trabalhar com coleções. | |
| [`dart:convert`][dart-convert], [`package:convert`][package-convert]<br>Encoders e decoders para converter entre diferentes representações de dados, incluindo JSON e UTF-8.<br>`package:convert` fornece encoders e decoders adicionais. ||
| [`dart:developer`][dart-developer]<br>Interação com ferramentas de desenvolvedor como o debugger e inspector. | Apenas [Native JIT][jit] e o [compilador JavaScript de desenvolvimento][] |
| [`dart:math`][dart-math]<br>Constantes e funções matemáticas, além de um gerador de números aleatórios. | |
| [`dart:typed_data`][dart-typed_data], [`package:typed_data`][package-typed_data]<br>Listas que manipulam eficientemente dados de tamanho fixo (por exemplo, inteiros sem sinal de 8 bytes) e tipos numéricos SIMD.<br>`package:typed_data` fornece classes e funções adicionais para trabalhar com dados tipados. | |
...
[plataforma nativa Dart](/overview#native-platform) (código compilado AOT e JIT).

| Library                                       | Notes                         |
|-----------------------------------------------|-------------------------------|
| [`dart:ffi`][dart-ffi], [`package:ffi`][package-ffi]<br>Uma interface de função externa que permite que o código Dart use APIs C nativas.<br>`package:ffi` contém utilitários incluindo suporte para converter strings Dart e strings C. | |
| [`dart:io`][dart-io], [`package:io`][package-io]<br>Suporte a arquivo, socket, HTTP e outras operações de I/O para aplicações não-web.<br>`package:io` fornece funcionalidades incluindo suporte para cores ANSI, cópia de arquivos e códigos de saída padrão. | |
| [`dart:isolate`][dart-isolate]<br> Programação concorrente usando isolates: workers independentes semelhantes a threads. | |
| [`dart:mirrors`][dart-mirrors]<br> Reflexão básica com suporte para introspecção e invocação dinâmica. | Experimental<br>Apenas [Native JIT][jit] (_não_&nbsp;Flutter) |
...
A tabela a seguir lista as bibliotecas principais do Dart que funcionam na
[plataforma web Dart](/overview#web-platform) (código compilado para JavaScript).
...
| Library                                       | Notes                         |
|-----------------------------------------------|-------------------------------|
| [**`package:web`**][pkg-web] <br>Bindings leves de API do navegador construídos em torno da interoperabilidade JS | Substitui todas as bibliotecas web `dart:*`. Leia o [guia de migração][html-web]. |
| [**`dart:js_interop`**][js-interop] <br>Interoperabilidade com JavaScript e APIs do navegador. | Substitui `package:js`. |
...
| [*`dart:html`*][dart-html] *(legado)* <br>Elementos HTML e outros recursos para aplicações baseadas na web. | Use `package:web` em vez disso. |
| [*`dart:indexed_db`*][dart-indexed_db] *(legado)* <br>Armazenamento chave-valor no lado do cliente com suporte para índices.  | Use `package:web` em vez disso. |
| [*`dart:js`*][dart-js], [*`dart:js_util`*][dart-js_util], [*`package:js`*][package-js] *(legado)* <br>Primitivas de baixo nível e anotações de alto nível para interoperabilidade JS. | Use `dart:js_interop` ou `dart:js_interop_unsafe` em vez disso. |
| [*`dart:svg`*][dart-svg] *(legado)* <br>Scalable Vector Graphics.  | Use `package:web` em vez disso. |
...
[compilador JavaScript de desenvolvimento]: /tools/webdev#serve
