...
ia-translate: true
title: dart:math
description: Aprenda sobre as principais funcionalidades da biblioteca dart:math do Dart.
prevpage:
...
máximo e mínimo, e constantes como *pi* e *e*. A maioria das
funcionalidades na biblioteca Math são implementadas como funções de nível superior.

Para usar esta biblioteca em sua aplicação, importe dart:math.
...
Gere números aleatórios com a classe [Random][]. Você pode
opcionalmente fornecer uma seed para o construtor Random.
...
Você pode até gerar booleans aleatórios:
...
que são inadequados para propósitos criptográficos.
...
Consulte a [referência da API Math][dart:math] para uma lista completa de métodos.
Veja também a referência da API para [num,][num] [int,][int] e [double.][double]
