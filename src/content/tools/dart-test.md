...
estão sob o diretório `test` do projeto Dart atual.
Para informações sobre como escrever testes, consulte a
[documentação de testes][testing documentation].
Se você está trabalhando em código Flutter, então use o comando `flutter test`,
conforme descrito em [Testando aplicativos Flutter][Testing Flutter apps].
...
que estão sob o diretório `test` do projeto atual:
...
diretórios ou arquivos sob o diretório `test`:
...
O comando `dart test` tem muitas outras flags para controlar
