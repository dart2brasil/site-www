...
A análise estática permite que você encontre problemas antes de
executar uma única linha de código. É uma ferramenta poderosa
usada para prevenir bugs e garantir que o código esteja em conformidade
com as diretrizes de estilo.
...
:::note
Para ver vários diagnósticos do analyzer com explicações e correções comuns,
...
- Use a entrada `analyzer:` para personalizar a análise estática:
  ...
  [alterando a severidade das regras](#changing-the-severity-of-rules), ou
...
Considere a seguinte estrutura de diretórios para um projeto grande:
...
o [sistema de tipos do Dart][type-system] exige,
...
Você pode usar os modos juntos ou separadamente; todos são `false` por padrão.

`strict-casts: <bool>`
: Um valor de `true` garante que o mecanismo de inferência de tipos nunca
  converta implicitamente de `dynamic` para um tipo mais específico.
  O seguinte código Dart válido inclui uma conversão implícita para baixo do
  ...
  exigindo que você adicione uma conversão explícita ou ajuste seu código de outra forma.
...
: Um valor de `true` garante que o mecanismo de inferência de tipos nunca escolha
  o tipo `dynamic` quando não conseguir determinar um tipo estático.
...
que resultam em falha de inferência.

Consulte [Condições para falha de inferência rigorosa][]
para uma lista exaustiva de condições de falha de inferência.
:::

[Condições para falha de inferência rigorosa]: {{site.repo.dart.lang}}/blob/main/resources/type-system/strict-inference.md#conditions-for-strict-inference-failure

`strict-raw-types: <bool>`
: Um valor de `true` garante que o mecanismo de inferência de tipos nunca escolha
  o tipo `dynamic` quando não conseguir determinar um tipo estático
  devido a argumentos de tipo omitidos.
  O seguinte código Dart válido tem uma variável `List` com um tipo raw,
  resultando em uma dica de tipo raw por este modo:
...
Por exemplo, você pode incluir o conjunto de regras recomendadas assim:
...
:::important
Quando uma **nova versão do `lints`** for publicada,
código que anteriormente passava na análise pode **começar a falhar na análise.**
Recomendamos atualizar seu código para funcionar com as novas regras.
Outras opções são habilitar explicitamente regras individuais do linter
ou [desabilitar regras individuais][].
:::

:::note
Para mais informações sobre incluir arquivos de opções,
confira a seção [Incluindo opções compartilhadas](#including-shared-options).
:::
...
seguida de `rules:` como uma chave de segundo nível.
...
### Desabilitando regras individuais
...
então cada linha deve conter o nome de uma regra seguido de
`: false` ou `: true`.

Aqui está um exemplo de um arquivo de opções de análise
que usa todas as regras recomendadas do `lints`
...
:::note
Devido a restrições do YAML,
**você não pode misturar sintaxe de lista e chave-valor na mesma entrada `rules`.**
Você pode usar a outra sintaxe para regras em um arquivo incluído.
:::

## Incluindo opções compartilhadas
...
Você pode especificar tais arquivos usando o campo de nível superior `include:`:
...
As opções em um arquivo incluído podem ser substituídas no arquivo incluidor,
assim como por arquivos incluídos subsequentes.
Em outras palavras, as opções especificadas por um arquivo de opções de análise são
calculadas primeiro aplicando as opções especificadas em cada um dos arquivos incluídos
(aplicando recursivamente este algoritmo), na ordem em que aparecem na lista,
e então substituindo-as por quaisquer opções definidas localmente.
...
E um arquivo de opções final que inclui estes:
...
## Habilitando plugins do analyzer (experimental) {:#plugins}

O analyzer tem suporte experimental para plugins.
Esses plugins se integram com o analyzer para adicionar funcionalidade
como novos diagnósticos, correções rápidas e autocompletar personalizado.
Você pode habilitar apenas um plugin por arquivo `analysis_options.yaml`.
Habilitar um plugin do analyzer aumenta a quantidade de memória que o analyzer usa.

Não use plugins do analyzer se sua situação atender
...
Você pode encontrar alguns plugins do analyzer no
[pub.dev]({{site.pub-pkg}}?q=dependency%3Aanalyzer_plugin).

Para habilitar um plugin:

 1. Adicione o pacote contendo o plugin como uma dependência de desenvolvimento.
 ...
 2. Edite seu arquivo `analysis_options.yaml` para habilitar o plugin.
    ...
    Para indicar funcionalidade específica do plugin a habilitar,
    como novos diagnósticos, pode ser necessária configuração adicional.

## Excluindo código da análise

Às vezes, é aceitável que algum código falhe na análise.
...
* Impedir que regras específicas não relacionadas a erros sejam aplicadas a arquivos individuais.
* Impedir que regras específicas não relacionadas a erros sejam aplicadas a linhas individuais de código.

Você também pode [desabilitar regras específicas][disable individual rules]
...
Para excluir arquivos da análise estática, use a opção `exclude:` do analyzer.
...
diretório contendo o arquivo `analysis_options.yaml`.
...
### Suprimindo diagnósticos para um arquivo

Para ignorar um diagnóstico específico não relacionado a erro para um arquivo específico,
...
Isso age para o arquivo inteiro, antes ou depois do comentário, e é
...
### Suprimindo diagnósticos para uma linha de código

Para suprimir um diagnóstico específico não relacionado a erro em uma linha específica de código Dart,
coloque um comentário `ignore` acima da linha de código.
Aqui está um exemplo de ignorar código que causa um erro de tempo de execução,
como você pode fazer em um teste de linguagem:
...
### Suprimindo diagnósticos em um arquivo pubspec

Se você precisar suprimir um diagnóstico não relacionado a erro do analyzer
...
pois deseja colocar a dependência `flutter` primeiro:
...
Se você estiver usando Dart 3.2 ou anterior,
o diagnóstico ignorado ainda será acionado.
:::

[`sort_pub_dependencies`]: /tools/linter-rules/sort_pub_dependencies

## Personalizando regras de análise

Cada [diagnóstico do analyzer][analyzer diagnostics] e
...
O analyzer suporta três níveis de severidade:

`info`
: Uma mensagem informativa que não causa falha na análise.
  Exemplo: [`dead_code`][dead_code]

`warning`
: Um aviso que não causa falha na análise a menos que
  o analyzer esteja configurado para tratar avisos como erros.
  Exemplo: [`invalid_null_aware_operator`][invalid_null_aware_operator]

`error`
: Um erro que causa falha na análise.
  Exemplo: [`invalid_assignment`][invalid_assignment]


### Ignorando regras

Você pode ignorar [diagnósticos do analyzer][] e [regras do linter][]
específicos usando o campo `errors:`.
Liste a regra, seguida de <code>:&nbsp;ignore</code>. Por exemplo, o seguinte
...
### Alterando a severidade das regras
...
e a fornecer informações (mas não um aviso ou erro) sobre código morto:
...
## Configurando `dart format`

Você pode configurar o comportamento do [`dart format`][] adicionando uma
seção `formatter` ao arquivo de opções de análise
especificando sua `page_width` preferida.

Para mais informações, leia [Configurando a largura de página do formatador][].

## Recursos

Use os seguintes recursos para aprender mais sobre análise estática em Dart:

* [Sistema de tipos do Dart][type-system]
* [Regras do linter Dart][linter rules]
* [pacote analyzer]({{site.pub-pkg}}/analyzer)
...
[Configurando a largura de página do formatador]: /tools/dart-format#configuring-formatter-page-width
