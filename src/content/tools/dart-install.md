...
ia-translate: true
title: dart install
description: Instalar ferramentas CLI Dart para uso global.
---
...
O comando `dart install` instala ferramentas CLI Dart para uso global.
...
O seguinte comando instala todos os executáveis especificados na
...
O seguinte comando especifica um pacote no site pub.dev
para instalar.
...
Os seguintes comandos podem ser usados para instalar um pacote
...
O seguinte exemplo instala o pacote `async_await` do
[GitHub][]:
...
Os seguintes comandos são úteis para instalar,
desinstalar e verificar o estado de instalação de uma
ferramenta CLI Dart.

### dart install

Instala um pacote para Dart.
...
| **`--git-ref`**        | O branch ou commit git específico a recuperar. Isso se aplica apenas ao usar uma URL git para `<package>`. |
| **`--overwrite`**      | Permite sobrescrever executáveis de outros pacotes que tenham o mesmo nome.                            |
...
Desinstalar um pacote para Dart.
...
Por exemplo, os seguintes comandos produzem uma visão geral e lista
...
Use a opção `--git-ref` para especificar o branch ou commit Git
a ser recuperado.
...
executáveis globais previamente instalados com o mesmo nome. Se você não especificar
esta flag, o executável preexistente não será substituído.
...
puxa a versão 0.6.0 do pacote `markdown`:

```console
$ dart install markdown 0.6.0
```

Se você especificar um intervalo, pub escolhe a melhor versão que atende essa
