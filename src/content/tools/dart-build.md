...
ia-translate: true
title: dart build
description: Ferramenta de linha de comando para construir aplicações Dart.
---
...
Use o comando `dart build` para construir uma aplicação Dart.
...
## Construir uma aplicação CLI

O comando `dart build cli`
constrói uma aplicação Dart com uma interface de linha de comando (CLI)
...
Use `-o` ou `--output` para especificar onde o `dart build` salva
...
Use a opção `-t` ou `--target` para especificar o arquivo de entry-point
...
arquivo Dart em `bin/`, então esse é usado.
...
Por exemplo, para definir o entrypoint como o arquivo `bin/my_command.dart`:
