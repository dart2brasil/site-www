...
description: >-
  ...
  dados JSON da API da Wikipedia.
sitemap: false
noindex: true
layout: learn
prevpage:
...
dados [JSON (JavaScript Object Notation)][JSON (JavaScript Object Notation)] em Dart.
...
* Entender a sintaxe básica do Dart, incluindo [classes][classes] e tipos de dados.
...
representar os dados JSON retornados pela API da Wikipedia.
Isso permitirá que você acesse e use facilmente os dados em sua aplicação.

### Tarefa 1: Criar o pacote Wikipedia

Primeiro, crie um novo pacote Dart para abrigar os modelos de dados.
...
1.  Execute o seguinte comando no seu terminal:
    ...
    de um pacote Dart. Você deve agora ver uma nova pasta
...
1.  **Adicione resolução de workspace aos sub-pacotes.**
    ...
    Isso diz ao Dart para resolver dependências dentro do workspace.
    ...
    *   Para `command_runner/pubspec.yaml`:
...
A API da Wikipedia retorna um objeto JSON contendo um resumo de um artigo.
...
1.  Adicione o seguinte código ao `wikipedia/lib/src/model/summary.dart`:
    ...
    campos na resposta JSON da API da Wikipedia.
    O método `fromJson` usa [pattern matching][pattern matching] para
    extrair os dados do objeto JSON e criar uma nova instância de `Summary`.
    O método `toString` fornece uma forma conveniente de
    imprimir o conteúdo do objeto `Summary`.
    Note que a classe `TitlesSet` é usada na classe `Summary`,
    então você precisará criá-la a seguir.
...
1.  Adicione o seguinte código ao `wikipedia/lib/src/model/title_set.dart`:
    ...
    informações de título na resposta JSON da API da Wikipedia.
...
A API da Wikipedia também retorna uma lista de artigos em um resultado de busca.
Vamos criar uma classe Dart para representar um artigo.

1.  Crie o arquivo `wikipedia/lib/src/model/article.dart`.

1.  Adicione o seguinte código ao `wikipedia/lib/src/model/article.dart`:
    ...
    o título e o extrato de um artigo.
...
Finalmente, vamos criar uma classe para representar os
resultados de busca da API da Wikipedia.

1.  Crie o arquivo `wikipedia/lib/src/model/search_results.dart`.
2.  Adicione o seguinte código ao `wikipedia/lib/src/model/search_results.dart`:
...
Não há nada para testar neste ponto.
Você adicionará essa lógica de aplicação nas próximas seções,
o que permitirá que você teste como os dados são desserializados da API da Wikipedia.
...
lógica de desserialização de JSON esteja funcionando corretamente.
