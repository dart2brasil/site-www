...
description: "Aprenda sobre programação assíncrona em Dart e como fazer requisições HTTP."
sitemap: false
noindex: true
layout: learn
prevpage:
...
Neste capítulo, você explorará a programação assíncrona em Dart, permitindo que suas
aplicações executem múltiplas tarefas concorrentemente. Você aprenderá como buscar
...
* Aprender sobre o pacote `http` e instruções import.
* Implementar a função `getWikipediaArticle` passo a passo, incluindo `http.Client`.
* Entender como a função de lógica principal se torna `async` para manipular `await`.
* Executar a aplicação para ver a busca de dados HTTP em ação.

:::

## Pré-requisitos

* Conclusão do Capítulo 2, que cobriu sintaxe básica Dart e interação de linha de
  comando. Você deve ter um projeto `dartpedia` configurado.
* Familiaridade com o conceito de APIs ([Application Programming Interfaces][]) como uma
  maneira de recuperar dados.

## Tarefas

Neste capítulo, você modificará a aplicação CLI `dartpedia` existente para
...
1.  Adicione `http: ^1.3.0` (ou a versão estável mais recente) sob `dependencies`. O
...
1.  Execute `dart pub get` no seu terminal do diretório `dartpedia/cli`. Este
...
1.  Adicione a seguinte instrução `import` no topo do arquivo, junto com o
    ...
    Esta linha importa o pacote `http` e lhe dá o alias `http`. Depois
    que você faz isso, pode se referir a classes e funções dentro do pacote `http`
...
Agora crie uma nova função chamada `getWikipediaArticle` que manipula
...
1.  **Construa a URL da API e `http.Client`:**
    Dentro da sua nova função `getWikipediaArticle`, crie uma instância de `http.Client()`
...
2.  **Faça a Requisição HTTP e manipule a resposta:**
    Agora, use o cliente `http` para fazer uma requisição HTTP `GET` para a URL que você acabou de construir. A keyword `await` pausa a execução de
    `getWikipediaArticle` até que a chamada `client.get(url)` complete e retorne
    um objeto `http.Response`.

    Após a requisição completar, verifique o `response.statusCode` para garantir que a
    requisição foi bem-sucedida (um código de status de `200` significa OK). Se bem-sucedido,
...
a lógica principal para manipular o comando `wikipedia`.

1.  **Atualize `searchWikipedia` para usar `async`:**
    ...
    Sua função `searchWikipedia` deve ficar assim (parte inicial):
    ...
    * `void searchWikipedia(List<String>? arguments) async`: A função agora é
    ...
    que é uma função `async` ela mesma e precisará fazer `await` do seu resultado.

1.  **Adicione verificações de `null` e string vazia para entrada do usuário:**
    Dentro de `searchWikipedia`, refine o bloco `if` que manipula o caso onde
...
1.  **Chame `getWikipediaArticle` e imprima o resultado:**
    ...
    instruções `print` de placeholder anteriores pela chamada real da API.
    ...
    * `await getWikipediaArticle(articleTitle)`: Como `getWikipediaArticle`
    é uma função `async`, você precisa fazer `await` do seu resultado. Isso pausa a
    função `searchWikipedia` até que o `Future<String>` retornado por
    `getWikipediaArticle` resolva em uma `String` contendo o conteúdo do
    artigo.
...
1.  Localize o bloco `else if` na sua função `main` que atualmente manipula
    ...
    o que significa que `main` ela mesma não precisa ser marcada como `async`.

    Sua função `main` deve ficar assim:
    ...
    * `arguments.sublist(1)`: Isso extrai todos os elementos da lista `arguments`
    ...
    o próprio comando `wikipedia`, então `searchWikipedia` recebe apenas os
    argumentos reais do título do artigo.
    * `searchWikipedia(inputArgs)`: Isso chama `searchWikipedia` diretamente. Como
    `main` não precisa fazer nada após `searchWikipedia` completar, você
    não precisa fazer `await` dela de `main` (e portanto `main` não precisa ser
    `async`).

### Tarefa 6: Executar a aplicação

Agora que você implementou a requisição `http` e a integrou na sua
aplicação, teste-a.
...
1.  Verifique para garantir que a aplicação buscou o resumo do artigo "Dart"
...
para seus arquivos Dart.
* **Requisições HTTP:** Fazer chamadas de rede usando a biblioteca `package:http`.
* **Interação com API:** Buscar dados de uma API pública (Wikipedia) e manipular
sua resposta.
...
bibliotecas e pacotes reutilizáveis. Você refatorará nossa aplicação para melhorar sua
