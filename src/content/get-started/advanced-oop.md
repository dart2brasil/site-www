---
ia-translate: true
title: Funcionalidades avançadas adjacentes à OOP
shortTitle: Funcionalidades OOP Avançadas
description: >-
  Aprimore suas habilidades em Dart explorando funcionalidades avançadas como enums aprimorados
...
Neste capítulo, você explorará funcionalidades avançadas do Dart que melhoram a experiência do usuário
da sua aplicação de linha de comando. Você aprenderá como usar enums
aprimorados para gerenciar cores do console e extensions para adicionar novas funcionalidades a
tipos existentes, tornando sua aplicação mais interativa e visualmente atraente.

:::secondary O que você aprenderá

*   Usar enums aprimorados para definir e gerenciar cores do console.
...
*   Ter completado o Capítulo 6 e ter um ambiente de desenvolvimento Dart funcionando
...
*   Ter um entendimento básico de conceitos de programação orientada a objetos como
    classes e enums.

## Tarefas

Você melhorará a experiência do usuário da sua aplicação Dartpedia CLI adicionando
cor à saída e melhorando a formatação de texto.

### Tarefa 1: Aprimorar o enum de cores do console
...
2.  Adicione o seguinte código para definir o enum `ConsoleColor`:
    ...
    Este enum define um conjunto de cores do console com seus valores RGB
...
3.  Adicione métodos ao enum `ConsoleColor` para aplicar cores ao texto:
    ...
    Esses métodos usam [códigos de escape ANSI][ANSI escape codes] para aplicar cores de primeiro plano e fundo
...
A seguir, crie uma extension na classe `String` para adicionar métodos utilitários
para aplicar cores do console e formatar texto.
...
1.  Adicione o seguinte código ao arquivo `command_runner/lib/src/console.dart`:
    ...
    Ela adiciona três métodos getter para aplicar cores do console:
    `errorText`, `instructionText` e `titleText`. Também adiciona um método para
...
uma boa prática implementar exemplos de uso de um pacote em Dart para
...
2.  Substitua o conteúdo do arquivo pelo seguinte código:
    ...
    baseado em sua posição na string. O método `run` usa os métodos getter `titleText`,
...
**Questão 1:** O que é um enum aprimorado em Dart?
...
*   D) Um enum que pode ser usado apenas em aplicações Flutter.

**Questão 2:** O que é uma extension do Dart?

*   A) Uma forma de adicionar novos métodos a classes existentes.
*   B) Uma forma de criar novas classes a partir de classes existentes.
*   C) Uma forma de definir novos operadores em Dart.
*   D) Uma forma de criar novas variáveis em Dart.

## Próxima lição

Na próxima lição, você aprenderá como melhorar ainda mais o
pacote `command_runner` polindo o `HelpCommand`, completando a
classe `CommandRunner`, adicionando o argumento `onOutput` e fornecendo um exemplo
completo.
