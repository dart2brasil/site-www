...
description: >-
  Melhore a robustez da aplicação tratando erros. Aprenda sobre exceptions, errors,
...
tratando erros de forma elegante. Você explorará exceptions, blocos `try/catch` e
como criar exceptions personalizadas para gerenciar erros de forma estruturada.

:::secondary O que você aprenderá

* Entender a diferença entre errors e exceptions em Dart.
* Usar blocos `try/catch` para tratar exceptions.
* Fazer `throw` de exceptions para sinalizar erros.
* Criar uma classe de exception personalizada.
* Usar `rethrow` de exceptions para propagá-las pela pilha de chamadas.
...
e adicionará tratamento de erros ao `CommandRunner` para gerenciar elegantemente os erros que
podem ocorrer durante a execução de comandos.
...
1.  Crie o arquivo `command_runner/lib/src/exceptions.dart`. Este arquivo
    conterá a definição da sua classe `ArgumentException`.

1.  Defina uma classe chamada `ArgumentException` que estende `FormatException`.
    ...
    Esta classe estende `FormatException`, que é uma classe de exception
    nativa do Dart. Ela inclui propriedades adicionais para armazenar o comando e o nome do argumento
    associado ao erro. Isso fornece mais contexto ao tratar a
    exception.
...
A seguir, atualize a classe `CommandRunner` para tratar potenciais erros
...
1.  Adicione um callback `onError` ao `CommandRunner`.
    ...
    Esta mudança introduz uma propriedade `onError` anulável. O tipo
    `FutureOr<void> Function(Object)?` significa que é uma função que recebe um
...
1.  Atualize o método run para usar `try`/`catch`.
    ...
    ocorrer, este bloco irá "capturá-la" e passá-la para o callback `onError`
    ou fazer rethrow dela se nenhum callback foi fornecido. `rethrow` preserva o
...
1.  Adicione validação ao método `parse`.
    ...
    Finalmente, substitua o método `parse` existente em `command_runner_base.dart`
    ...
    Este método parse atualizado agora se defende ativamente contra entradas ruins.
    Especificamente, as novas declarações throw lidam com vários casos comuns de erro:
    ...
    *   Múltiplos comandos: Verifica que o usuário não tentou executar mais de
        um comando por vez.
    *   Options desconhecidas: O parâmetro `orElse` dentro de `firstWhere` agora lança uma
        exception se um usuário fornecer uma flag ou option (como `--foo`) que
        não foi definida para aquele comando.
    *   Valores de option faltando: Garante que uma option (como `--output`) seja
        seguida por um valor e não por outra option ou pelo fim da entrada.
    *   Muitos argumentos: Impõe uma regra de que comandos podem ter apenas um
...
1.  Atualize a função `main` para passar um método `onError` ao
    ...
    Este código passa uma função callback `onError` ao construtor do `CommandRunner`.
    Se um erro ocorrer durante a execução de um comando, a
    função callback `onError` é chamada com o objeto de erro. O callback
    verifica se o erro é um `Error` ou uma `Exception`. Se for um
    `Error`, é relançado. Se for uma `Exception`, é impresso no console.

### Tarefa 4: Atualizar exports da biblioteca command_runner

Torne `ArgumentException` disponível para a biblioteca `command_runner`.

1.  Abra `command_runner/lib/command_runner.dart` e adicione os seguintes
    exports:
    ...
    Isso garante que a `ArgumentException` esteja disponível para os consumidores do
...
* B) Tratar exceptions que podem ocorrer durante a execução do código.
...
Na próxima lição, você aprenderá sobre funcionalidades avançadas orientadas a objetos em
