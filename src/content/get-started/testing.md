...
Neste capítulo, você aprenderá como escrever testes para seu código Dart. Testar é
...
*   Escrever testes para lógica de desserialização de JSON.
...
A seguir, crie um arquivo de teste para seus modelos de dados e adicione os imports necessários a
ele.
...
1.  A seguir, crie um arquivo chamado `cat_extract.json`. Este arquivo é muito longo, então copie
...
1.  A seguir, crie um arquivo chamado `open_search_response.json` e cole este conteúdo
    nele:
...
Com esses arquivos em ordem, você está pronto para escrever os testes que verificarão seus
modelos de dados.

### Tarefa 4: Escrever testes para desserialização de JSON

Agora, você escreverá testes para a lógica de desserialização de JSON em seus modelos de dados.
Você usará as funções `group`, `test` e `expect` do pacote `test`.

1.  Use a função `group` para agrupar testes relacionados juntos.
...
1.  Crie um teste para o modelo `Summary`.
    ...
    *   Cria um objeto `Summary` do map usando o construtor
        `Summary.fromJson`.
    *   Usa a função `expect` para afirmar que a propriedade `canonical` do
    ...
    afirmar que o valor atende certos critérios. Neste caso, o
...
1.  Crie um teste para o modelo `Article`. Adicione a seguinte função `test`
    ...
    *   Decodifica a string JSON em um `List<Object?>`.
    *   Cria o objeto `Article` da lista usando o construtor
...
1.  Crie um teste para o modelo `SearchResults`.
    ...
    *   Decodifica a string JSON em um `List<Object?>`.
    *   Cria um objeto `SearchResults` da lista usando o construtor
...
Agora que você escreveu os testes, você pode executá-los para verificar se eles passam.
...
*   Criar arquivos de teste e organizar testes.
*   Escrever testes para lógica de desserialização de JSON.
*   Usar matchers para afirmar que valores atendem certos critérios.
...
*   B) Agrupar testes relacionados juntos.
*   C) Definir um novo caso de teste.
*   D) Afirmar que um valor atende certos critérios.
...
*   B) Agrupar testes relacionados juntos.
*   C) Afirmar que um valor atende certos critérios.
*   D) Executar todos os testes em um projeto.

## Próxima lição

Na próxima lição, você implementará as chamadas à API da Wikipedia em sua
