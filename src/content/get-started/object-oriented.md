---
ia-translate: true
title: Programação Dart orientada a objetos
...
no Dart. Você aprenderá como criar classes e definir relacionamentos entre
...
* Implementar herança usando a keyword `extends` e sobrescrever métodos.
* Usar `FutureOr` para funções que podem retornar um valor síncrona ou
  assincronamente.
...
* Ter completado o Capítulo 4 e ter um ambiente de desenvolvimento Dart funcionando com
  o projeto `dartpedia`.
...
Aqui está uma análise de cada parte:

* `dartpedia`: Este é o **executável**, o nome do seu aplicativo.
* `help`: Este é um **comando**, uma ação que você quer que o aplicativo execute.
* `--verbose`: Esta é uma **flag** (um tipo de opção que não recebe um valor),
...
As classes e lógica que você construir nas tarefas seguintes criam a base para
analisar e executar comandos exatamente como este.

### Tarefa 1: Definir a hierarquia de argumentos

Primeiro, você definirá uma classe `Argument`, uma classe `Option` e uma classe `Command`,
estabelecendo um relacionamento de herança.
...
2.  Defina um `enum` chamado `OptionType`.
    ...
    Este `enum` representa o tipo de opção, que pode ser ou uma **`flag`**
    (uma opção boolean) ou uma **`option`** regular (uma opção que recebe um valor).
    Enums são úteis para representar um conjunto fixo de valores possíveis.

3.  Defina uma `abstract class` chamada `Argument`.

    Comece definindo a estrutura básica da sua classe `Argument`. Você
    a declarará como `abstract`, o que significa que ela serve como um blueprint que outras
    classes podem estender, mas não pode ser instanciada por si só.
    ...
    * O getter **`usage`** fornecerá uma string mostrando como usar o
    ...
    Com a classe `Argument` totalmente definida, você tem uma interface comum para todos
    os tipos de argumentos de linha de comando. Em seguida, você construirá sobre isso definindo
...
4.  Defina uma classe chamada `Option` que `extends` `Argument`.
    ...
    A keyword **`extends`** estabelece o relacionamento de herança. O
    ...
    representará as ações principais que um usuário pode executar no seu aplicativo CLI.
...
5.  Defina uma `abstract class` chamada `Command` que também `extends` `Argument`.
    ...
    diretamente. Ela serve como um blueprint para outras classes.
    ...
    mais tarde em `command_runner_base.dart`. Para tornar o Dart ciente desta classe,
    você deve importar seu arquivo de definição. Adicione o seguinte import no topo de
    ...
    e exporá uma visão **somente leitura, não modificável** dela. Isso usa a
    classe `UnmodifiableSetView`, que faz parte da biblioteca de coleção
    central do Dart. Para usá-la, você deve importar essa biblioteca.

    Atualize os imports no topo do seu arquivo para incluir `dart:collection`:
    ...
    Finalmente, todo comando deve ter lógica para executar. O método `run` deve
    ser flexível, permitindo retornar um valor imediatamente
    (sincronamente) ou após um atraso (assincronamente). O tipo `FutureOr<Object?>`
    da biblioteca `async` do Dart serve a esse propósito. Isso significa que o método
    deve retornar um valor (de qualquer tipo ou `null`) sincronamente ou
    assincronamente. Este é o seu import final necessário.

    Atualize os imports no topo do seu arquivo para incluir `dart:async`:
    ...
    Agora você pode adicionar o método abstrato `run` e fornecer a implementação de `usage`
    ...
    * **`run(ArgResults args)`**: Este método abstrato é onde a lógica de um
      comando reside. Subclasses concretas *devem* implementá-lo.
    * **`usage`**: Este getter fornece uma string de uso simples,
      combinando o `name` e `description` do comando.

    A classe `Command` agora fornece uma base robusta para todos os comandos no
    seu aplicativo CLI. Com a hierarquia de classes no lugar, você está pronto para definir
    `ArgResults` para armazenar a entrada analisada.
...
6.  Defina uma classe chamada `ArgResults`.
    ...
    armazena o comando detectado, qualquer argumento para esse comando e um mapa de
    opções. Ela também fornece métodos auxiliares convenientes como `flag`,
    `hasOption` e `getOption`.

    Agora você definiu a estrutura básica para lidar com comandos, argumentos
    e opções no seu aplicativo de linha de comando.
...
3.  Abra `command_runner/lib/command_runner.dart`, e adicione as seguintes
    exportações:
    ...
    `help_command.dart` disponíveis para outros pacotes que dependem de
    `command_runner`.

### Tarefa 3: Criar um HelpCommand

Crie um `HelpCommand` que estende a classe `Command` e imprime informações
de uso.
...
2.  Substitua o código existente pelo seguinte:
    ...
    usando encadeamento de método (`..addCommand`), e então executa o command runner
...
* Definir e usar tipos **`enum`** para representar um conjunto fixo de valores.
* Construir um framework básico de análise de argumentos de linha de comando usando princípios OOP.

## Quiz

**Questão 1:** Na classe `Option`, qual é o propósito da
  notação `@override`?

* A) Criar um novo método que não existe na classe pai.
* B) Indicar que um método é opcional.
* C) Fornecer uma implementação específica para um método ou propriedade definida em uma
  classe pai.
* D) Tornar uma propriedade privada.

**Questão 2:** Qual é a diferença entre uma classe `abstract` e uma classe
regular no Dart?
...
**Questão 3:** No guia, para que foi usado o `enum OptionType`?
...
* D) Para controlar a cor da saída de linha de comando.

## Próxima lição

Na próxima lição, você aprenderá como lidar com erros e exceções no seu
