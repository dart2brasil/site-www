...
Neste capítulo, você implementará a funcionalidade principal do CLI do Wikipedia
fazendo chamadas de API para recuperar dados. Você adicionará o pacote `http` como uma
...
*   Entender conceitos básicos de redes (como APIs e requisições HTTP).
*   Entender formatos básicos de serialização de dados como JSON.

## Tarefas

Neste capítulo, você adicionará a funcionalidade principal do CLI do Wikipedia
fazendo chamadas de API para recuperar dados. Você trabalhará dentro do pacote `wikipedia` para
...
1.  Adicione `http: ^1.3.0` (ou a versão estável mais recente) em `dependencies`.
...
### Tarefa 2: Implementar chamadas à API do Wikipedia

Em seguida, você criará as funções de API para buscar dados do Wikipedia. Você
criará três arquivos:

* `summary.dart`: Este arquivo conterá funções para recuperar resumos
  de artigos.
* `search.dart`: Este arquivo lidará com consultas de busca para encontrar artigos.
* `get_article.dart`: Este arquivo conterá funções para buscar o
  conteúdo completo de um artigo.

1.  Crie o diretório `wikipedia/lib/src/api`.

1.  Crie o arquivo `wikipedia/lib/src/api/summary.dart`.

1.  Adicione o seguinte código a `wikipedia/lib/src/api/summary.dart`:
    ...
    requisições GET à API do Wikipedia e retornar um objeto `Summary`.
    `getRandomArticleSummary` busca um resumo para um artigo aleatório, enquanto
    `getArticleSummaryByTitle` busca um resumo para um título de artigo específico.

1.  Em seguida, crie o arquivo `wikipedia/lib/src/api/search.dart`.

1.  Adicione o seguinte código a `wikipedia/lib/src/api/search.dart`:
    ...
    `opensearch` da API do Wikipedia e retorna um objeto `SearchResults`.
    O endpoint `opensearch` é usado para buscar
    artigos do Wikipedia baseado em um termo de busca.

1.  Crie o arquivo `wikipedia/lib/src/api/get_article.dart`.

1.  Adicione o seguinte código a `wikipedia/lib/src/api/get_article.dart`:
    ...
    para fazer uma requisição GET à API do Wikipedia e retorna um
    objeto `List<Article>`. Esta função recupera o conteúdo de um artigo
    do Wikipedia baseado em seu título.
...
1.  Adicione as seguintes instruções `export` ao arquivo:
    ...
    Estas instruções `export` tornam as funções de API e modelos disponíveis para
...
dependências do pacote, é uma boa prática executar os testes que você criou no
capítulo anterior. Isso confirmará que suas alterações não quebraram a funcionalidade
...
1.  Remova o arquivo de teste padrão executando o comando `rm wikipedia_test.dart`
    ...
    gerado automaticamente mas não é usado em nosso projeto.

1.  Abra seu terminal e navegue até o diretório `wikipedia`.

1.  Execute o comando `dart test`.

    Você deverá ver uma saída similar a esta, confirmando que todos os seus testes existentes
    ainda passam:
...
**Questão 3:** Qual é o propósito da instrução `export` em uma biblioteca Dart?
...
Na próxima lição, você completará o CLI integrando o pacote `wikipedia`
com o pacote `cli`. Você implementará a lógica de comandos e exibirá
